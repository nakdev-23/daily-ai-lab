-- Migration 040: enforce the free preview cap inside complete_lesson
-- Free career-path preview lets a Free user learn the first N steps of a Pro
-- path (UI gate in paths-learn). complete_lesson is a public authenticated RPC,
-- so the same cap must be enforced here too — otherwise a crafted request could
-- complete later steps (earning XP/progress and eventually triggering the
-- certificate from migration 024) without Pro. Mirrors the codebase posture of
-- enforcing every gate at the data layer, not just the page.
--
-- IMPORTANT: keep this in sync with FREE_CAREER_PREVIEW_STEPS in
-- apps/web/lib/career-preview.ts (currently 3).
...
declare
  ...
  v_course_uuid   uuid;
  v_path_slug     text;
  v_path_is_pro   boolean := false;
  v_preview_cap   int := 3;  -- keep in sync with FREE_CAREER_PREVIEW_STEPS
begin
  ...
  if p_course_id like 'path:%' then
    -- Career-path key: validate against the path's published step count and
    -- capture whether it is a Pro path (for the preview cap below).
    v_path_slug := substring(p_course_id from 6);
    select count(*)::int, bool_or(cp.is_pro) into v_max_lessons, v_path_is_pro
    from path_steps s
  ...
  -- NEW (040): free preview cap. On a Pro path, a Free user may only complete the
  -- first N steps. This stops a crafted call from earning XP/progress (and the
  -- auto-certificate) for content that is behind the paywall.
  if v_path_slug is not null and v_path_is_pro and not v_is_pro and p_lesson_num > v_preview_cap then
    return jsonb_build_object('ok', false, 'xp', 0, 'reason', 'preview-locked');
  end if;
