...
export async function saveSystemAction(_prev: SystemResult, formData: FormData): Promise<SystemResult> {
  ...
  const s: SystemSettings = {
    ...
    proPriceMonth: int(formData.get("pro_price_month"), 299, 0, 1000000),
    proPriceYear: int(formData.get("pro_price_year"), 2870, 0, 10000000),
    heartsResetHour: int(formData.get("hearts_reset_hour"), 0, 0, 23),
    freeLessonsPerDay: int(formData.get("free_lessons_per_day"), 2, 0, 99),
